...
void MainWindow::createLayout()
{
    ...
    conductionVelocityButton = new QPushButton(tr("Conduction Velocity"));
    ...
    numWaveformsLayout->addWidget(conductionVelocityButton);

    connect(spikeScopeButton, SIGNAL(clicked()),
            this, SLOT(spikeScope()));
    connect(conductionVelocityButton, SIGNAL(clicked()),
            this, SLOT(conductionVelocity()));
...
void MainWindow::conductionVelocity()
{
    if (!conductionVelocityDialog) {
        conductionVelocityDialog = new ConductionVelocityDialog(signalProcessor, signalSources,
                                                wavePlot->selectedChannel(), this);
        // add any 'connect' statements here
    }

    conductionVelocityDialog->show();
    conductionVelocityDialog->raise();
    conductionVelocityDialog->activateWindow();
    conductionVelocityDialog->setYScale(yScaleComboBox->currentIndex());
    conductionVelocityDialog->setSampleRate(boardSampleRate);
    wavePlot->setFocus();
}
...
void MainWindow::newSelectedChannel(SignalChannel* newChannel)
{
    ...
    if (conductionVelocityDialog){
        conductionVelocityDialog->setNewChannel(newChannel);
    }
